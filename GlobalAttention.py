...
class SentenceAttention(nn.Module):
    ...
    def forward(self, input, context):
        ...
        ih, iw = input.size(2), input.size(3)
        queryL = ih * iw
        batch_size, idf = context.size(0), input.size(1)

        # --> batch x idf x queryL
        target = input.view(batch_size, -1, queryL)
        # batch x cdf --> batch x cdf x 1 x 1
        sourceT = context.unsqueeze(2).unsqueeze(3)
        # --> batch x idf x 1
        sourceT = self.conv_context(sourceT).squeeze(3)

        # Get attention
        # (batch x idf x queryL) * (batch x idf x 1)
        # --> batch x idf x queryL
        attn = target * sourceT
        # --> batch*idf x queryL
        attn = attn.view(batch_size*idf, queryL)
        attn = self.sm(attn)  # Eq. (5)
        # --> batch x idf x queryL
        attn = attn.view(batch_size, idf, queryL)

        # (batch x idf x 1) * (batch x idf x queryL)
        # --> batch x idf x queryL
