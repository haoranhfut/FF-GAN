...
from GlobalAttention import WordAttention, SentenceAttention
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.affine = ACM(ngf)
        self.w_att = WordAttention(ngf, self.ef_dim)
        self.s_att = SentenceAttention(ngf, self.ef_dim)


    def forward(self, h_code, c_code, word_embs, sent_emb, mask, cap_lens):
        """
        ...
        h_c_code = self.affine(h_code, c_code)

        s_code, s_att = self.s_att(h_code, sent_emb)
        h_c_c_img_code = torch.cat((h_c_code, s_code), 1)
        out_code = self.residual(h_c_c_img_code)

        # state size ngf/2 x 2in_size x 2in_size
        out_code = self.upsample(out_code)

        return out_code, w_att
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask, cap_lens):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            h_code2, att1 = self.h_net2(h_code1, c_code, word_embs, sent_emb, mask, cap_lens)
            fake_img2 = self.img_net2(h_code2)
            fake_imgs.append(fake_img2)
            if att1 is not None:
                att_maps.append(att1)
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3, att2 = self.h_net3(h_code2, c_code, word_embs, sent_emb, mask, cap_lens)
