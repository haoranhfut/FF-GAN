...
class condGANTrainer(object):
    ...
    def save_img_results(self, real_imgs, netG, noise, sent_emb, words_embs, mask,
                         image_encoder, captions, cap_lens,
                         gen_iterations, real_image, name='current'):
        # Save images
        fake_imgs, attention_maps, _, _ = netG(noise, sent_emb, words_embs, mask, cap_lens)
        save_img_results_Img(real_imgs, fake_imgs, 3, gen_iterations, self.image_dir)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                errG_total += kl_loss
                G_logs += 'kl_loss: %.4f ' % kl_loss.item()
                # backward and update parameters
                errG_total.backward()
                optimizerG.step()
                ...
                if gen_iterations % 200 == 0:
                    backup_para = copy_G_params(netG)
                    load_params(netG, avg_param_G)
                    self.save_img_results(imgs, netG, fixed_noise, sent_emb, words_embs, mask, image_encoder,
            ...
            print('''[%d/%d] Loss_D: %.4f Loss_G: %.4f Time: %.4fs''' % (
            ...
            with torch.no_grad():
                for ii in range(11):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                    ...
                    for step, data in enumerate(self.data_loader, 0):
                        ...
                        ######################################################
                        noise.data.normal_(0, 1)
                        fake_imgs, _, _, _ = netG(noise, sent_emb, words_embs, None, cap_lens)
